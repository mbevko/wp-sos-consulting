<?php
/*
 * Template Name: ROI Calculator Distribution
 * Description: Distribution ERP Cost of Inaction Calculator — standalone, no site header/footer
 */
?><!DOCTYPE html>
<html lang="en">
<head>
...
<title>Distribution ERP Cost of Inaction Calculator — SOS Consulting Services</title>
...
<div class="topbar">
  ...
  <a href="<?php echo esc_url( home_url('/solutions/acumatica/distribution/') ); ?>" class="topbar-back">&larr; Back to Distribution</a>
</div>

<div class="hero">
  <div class="hero-inner">
    <div class="hero-tag"><div class="hero-dot"></div> Free Tool for Distributors</div>
    <h1>Distribution ERP<br><em>Cost of Inaction Calculator</em></h1>
    <p>Estimate the annual cost of manual distribution processes — order entry, warehouse operations, inventory reconciliation, and more. Takes 3 minutes. Results are yours to keep.</p>
  </div>
</div>
...
<div class="screen active" id="s-gate">
  <div class="gate-card">
    ...
    <div class="gate-body">
      <p class="gate-intro">This calculator estimates the <strong>soft cost savings</strong> your distribution business could realize by replacing manual processes with Acumatica Distribution Edition. Enter your info below to unlock the calculator and receive your results by email.</p>
      <div class="form-grid">
        ...
        <div class="form-group"><label class="form-label">Company</label><input class="form-input" type="text" id="company" placeholder="Acme Distribution" autocomplete="organization"></div>
        <div class="form-group">
          <label class="form-label">Industry</label>
          <select class="form-select" id="industry">
            <option value="">Select your industry</option>
            <option>Wholesale Distribution</option><option>Food &amp; Beverage Distribution</option>
            <option>Industrial / MRO Distribution</option><option>Consumer Goods Distribution</option>
            <option>Medical / Healthcare Distribution</option><option>Building Materials Distribution</option>
            <option>Technology / Electronics Distribution</option><option>Other Distribution</option>
          </select>
        </div>
        <div class="form-group">
...
<div class="screen" id="s-calc">
  <div class="calc-hdr">
    <div><h2>Your ROI Calculator</h2><p>Drag each slider to match your current manual distribution process hours</p></div>
...
<div class="screen" id="s-results">
  <div class="res-hero">
    ...
    <h2>Here's what your distribution operation<br>is <em>actually costing you.</em></h2>
  ...
  <div class="res-body">
    <div>
      ...
      <div class="disc"><strong>About these estimates:</strong> Calculations use process-specific reduction benchmarks ranging from 55% to 80%, based on Acumatica customer outcomes for distribution operations. Actual savings vary by company size, current system maturity, number of channels, warehouse complexity, and implementation scope. This tool produces directional estimates to inform the ROI conversation, not to guarantee specific savings. For a scoped estimate based on your actual operation, book a free assessment.</div>
    </div>
    <div class="res-right">
      <div class="res-cta">
...
<script>
var PROCS = [
  {id:"p1",label:"Manual order entry & processing",desc:"Keying orders from email, fax, phone, or EDI into your system manually",weekly:true,def:8,max:40,r:0.75},
  {id:"p2",label:"Inventory reconciliation",desc:"Cycle counts, adjustments, discrepancy research across warehouse locations",weekly:true,def:7,max:40,r:0.65},
  {id:"p3",label:"Warehouse receiving & put-away",desc:"Paper-based receiving, manual location assignment, data entry after the fact",weekly:true,def:10,max:40,r:0.80},
  {id:"p4",label:"Pick, pack & ship data entry",desc:"Manual pick list generation, packing slips, shipping label entry without automation",weekly:true,def:9,max:40,r:0.75},
  {id:"p5",label:"Purchase order creation & follow-up",desc:"Manual PO entry, supplier chasing, tracking delivery confirmations",weekly:true,def:5,max:40,r:0.60},
  {id:"p6",label:"Customer order status calls",desc:"Time spent answering where-is-my-order calls because customers lack live visibility",weekly:true,def:6,max:40,r:0.55},
  {id:"p7",label:"Month-end close extra time",desc:"Manual reconciliations, journal entries, and corrections vs. an automated system",weekly:false,def:20,max:80,r:0.70},
  {id:"p8",label:"Channel reconciliation",desc:"Manually reconciling orders and inventory from Amazon, Shopify, EDI, or other channels",weekly:false,def:10,max:40,r:0.65}
];

var userData = {};
var HS_PORTAL_ID = '45057611';
var HS_FORM_GUID = 'YOUR_DISTRIBUTION_ROI_FORM_GUID'; // Replace after creating "Distribution ROI Calculator Gate" form in HubSpot
...
function submitGate(fname, lname, email, company, onSuccess, onError) {
  ...
  fetch(url, {
    ...
    body: JSON.stringify({
      fields: [
        ...
        { name: 'lead_source', value: 'Distribution ROI Calculator' }
      ],
