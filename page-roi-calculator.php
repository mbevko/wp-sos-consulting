 ...
 * Description: Manufacturing ERP Soft Cost ROI Calculator — standalone, no site header/footer
 */
?><!DOCTYPE html>
<html lang="en">
<head>
...
<title>ERP Soft Cost ROI Calculator — SOS Consulting Services</title>
...
<div class="topbar">
  ...
  <a href="<?php echo esc_url( home_url('/solutions/acumatica/manufacturing/') ); ?>" class="topbar-back">&larr; Back to Manufacturing</a>
</div>

<div class="hero">
  <div class="hero-inner">
    <div class="hero-tag"><div class="hero-dot"></div> Free Tool</div>
    <h1>ERP Soft Cost<br><em>ROI Calculator</em></h1>
    <p>Estimate the annual cost of your current manual processes — and how much a modern ERP could save. Takes 3 minutes. Results are yours to keep.</p>
  </div>
</div>
...
<div class="screen active" id="s-gate">
  <div class="gate-card">
    ...
    <div class="gate-body">
      <p class="gate-intro">This calculator estimates the <strong>soft cost savings</strong> your business could realize by replacing manual ERP-adjacent processes with Acumatica. Enter your info below to unlock the calculator and receive your results by email.</p>
      <div class="form-grid">
        ...
        <div class="form-group"><label class="form-label">Company</label><input class="form-input" type="text" id="company" placeholder="Acme Manufacturing" autocomplete="organization"></div>
        <div class="form-group">
          <label class="form-label">Industry</label>
          <select class="form-select" id="industry">
            <option value="">Select your industry</option>
            <option>Discrete Manufacturing</option><option>Food &amp; Beverage Manufacturing</option>
            <option>Wholesale Distribution</option><option>Metal Fabrication</option>
            <option>Industrial Equipment</option><option>Consumer Goods</option>
            <option>Other Manufacturing</option><option>Other Distribution</option>
          </select>
        </div>
        <div class="form-group">
...
<div class="screen" id="s-calc">
  <div class="calc-hdr">
    <div><h2>Your ROI Calculator</h2><p>Drag each slider to match your current manual process hours</p></div>
...
<div class="screen" id="s-results">
  <div class="res-hero">
    ...
    <h2>Here's what your manual processes<br>are <em>actually costing you.</em></h2>
  ...
  <div class="res-body">
    <div>
      ...
      <div class="disc"><strong>About these estimates:</strong> Calculations assume a 65% average reduction in manual process time — a conservative benchmark based on Acumatica customer outcomes. Actual savings vary by company size, current system maturity, and implementation scope. This tool produces directional estimates to inform the ROI conversation, not to guarantee specific savings. For a scoped estimate based on your actual operation, book a free assessment.</div>
    </div>
    <div class="res-right">
      <div class="res-cta">
...
<script>
var PROCS = [
  {id:"p1",label:"Production scheduling & planning",desc:"MRP runs, capacity checks, expediting, adjustments",weekly:true,def:8,max:40,r:0.70},
  {id:"p2",label:"Inventory reconciliation",desc:"Cycle counts, adjustments, discrepancy research",weekly:true,def:6,max:40,r:0.65},
  {id:"p3",label:"Shop floor data entry",desc:"Paper travelers, manual labor recording, production entry",weekly:true,def:10,max:40,r:0.80},
  {id:"p4",label:"Excel reporting & data exports",desc:"Building reports, copying data between systems",weekly:true,def:7,max:40,r:0.75},
  {id:"p5",label:"Purchase order creation & follow-up",desc:"Manual PO entry, supplier chasing, delivery confirmation",weekly:true,def:5,max:40,r:0.60},
  {id:"p6",label:"Month-end close extra time",desc:"Manual reconciliations, journal entries, corrections",weekly:false,def:20,max:80,r:0.70},
  {id:"p7",label:"Ad-hoc reporting & data requests",desc:"Pulling custom data for management, board reports",weekly:false,def:8,max:40,r:0.65},
  {id:"p8",label:"Customer order support calls",desc:"Answering order status calls due to lack of live visibility",weekly:false,def:6,max:40,r:0.55}
];

var userData = {};
var HS_PORTAL_ID = '45057611';
var HS_FORM_GUID = 'YOUR_MANUFACTURING_ROI_FORM_GUID'; // Replace after creating "ROI Calculator Gate" form in HubSpot
...
function submitGate(fname, lname, email, company, onSuccess, onError) {
  ...
  fetch(url, {
    ...
    body: JSON.stringify({
      fields: [
        ...
        { name: 'lead_source', value: 'ROI Calculator' }
      ],
  ...
  .then(function(res) {
    ...
    else { onSuccess(); } // proceed even on non-critical errors
  })
  .catch(function() { onSuccess(); }); // don't block user if HubSpot is unreachable
}
